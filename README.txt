...
Obs: Foi requisitado uma aplicação web mas como já tinhamos começado a desenvolver em react-native, optamos por continuar o projeto assim,
	nota-se também que o projeto não está se comunicando com um banco de dados, não foi implementado pois não era preciso um para que 
	o sistema fizesse o que foi proposto, que é a busca de cep, todavia, nota-se que no DockerCompose tem uma configuração para banco
	de dado do SSMS-SqlServer, trazendo assim a possibilidade da configuração de um, caso a aplicação tivesse que usar.
